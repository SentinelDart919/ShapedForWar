...
public class TerraPlanetGenerator extends PlanetGenerator {
    ...
    Block[][] mapfloor =
            {
                    {Blocks.deepwater, Blocks.water, Blocks.darksandWater, Blocks.darksandWater, Blocks.stone, Blocks.basalt},
                    {Blocks.deepwater, Blocks.water, Blocks.darksandWater, Blocks.darksand, Blocks.basalt, Blocks.stone},
                    {Blocks.water ,Blocks.darksandWater, Blocks.darksand, Blocks.basalt, Blocks.stone, Blocks.stone},
                    {Blocks.darksandWater, Blocks.darksand, Blocks.basalt, Blocks.basalt, Blocks.stone, Blocks.stone},
                    {Blocks.water, Blocks.basalt, Blocks.stone, Blocks.snow, Blocks.iceSnow, Blocks.ice},
                    {Blocks.water, Blocks.basalt, Blocks.stone, Blocks.snow, Blocks.iceSnow, Blocks.ice},
                    {Blocks.water, Blocks.stone, Blocks.stone, Blocks.basalt, Blocks.slag, Blocks.darksandWater},
            };
    ...
    float water = 2f / mapfloor[0].length;
    @Override
    ...
    public void genTile(Vec3 position, TileGen tile){
        ...
        else if(tile.block == Blocks.water) tile.block = Blocks.air;
    ...
    Block getBlock(Vec3 position, boolean visualOnly){
        ...
        Block res = mapfloor[Mathf.clamp((int)(temp * mapfloor.length), 0, mapfloor[0].length - 1)][Mathf.clamp((int)(height * mapfloor[0].length), 0, mapfloor[0].length - 1)];
        if(tar > 0.5f){
    ...
    protected void generate() {
                        ...
                        for(int y = -rad; y <= rad; y++){
                            ...
                            if(Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)){
                                ...
                                if(Mathf.within(x, y, rad - 1) && !other.floor().isLiquid){
                                    ...
                                    other.setFloor((Floor)(floor == Blocks.darksand || floor == Blocks.basalt ? Blocks.darksandWater : Blocks.water));
                                    other.setBlock(Blocks.air);
                                }
                            }
                        }
        ...
        pass((x, y) -> {
            ...
            if(value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)){
                ...
                boolean salt = floor != Blocks.darksand && floor != Blocks.salt;
                ...
                if(!(floor == Blocks.ice || floor == Blocks.iceSnow || floor == Blocks.snow || floor.asFloor().isLiquid)){
                    floor = salt ?
                            (deep ? Blocks.deepwater : Blocks.water) :
                            (deep ? Blocks.deepwater :
                                    (floor == Blocks.darksand || floor == Blocks.salt ? Blocks.darksandWater : Blocks.water));
                }
            }
        });
        ...
        /*Seq<Block> ores = Seq.with(Blocks.stone, RebornOres.oreIron, Blocks.oreCoal);
        ...
        if(Simplex.noise3d(seed, 2, 0.5, scl, sector.tile.v.x + 1, sector.tile.v.y, sector.tile.v.z)*nmag + poles > 0.7f*addscl){
            ores.add(RebornOres.oreUranium);
        }
        ...
        pass((x, y) -> {
            ...
            for(int i = ores.size - 1; i >= 0; i--){
                ...
                if(Math.abs(0.5f - noise(offsetX, offsetY + i*999, 2, 0.7, (40 + i * 2))) > 0.22f + i*0.01 &&
                    ...
                    if(entry == Blocks.stone) floor = entry;
                    else ore = entry;
                    break;
                }
            }
        ...
        pass((x, y) -> {
            ...
            if(floor == Blocks.hotrock){
                ...
                if(noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))){
                    if(noise > 0.72f){
                        floor = noise > 0.78f ? Blocks.water : (floor == Blocks.darksand ? Blocks.darksandWater : Blocks.darksandWater);
                    }else{
                        floor = (floor == Blocks.darksand ? floor : Blocks.darksandWater);
                    }
                }
            }
